...
export default function TeamNominations() {
  ...
  const nominateMutation = useMutation({
    mutationFn: async () => {
      ...
      // Process each request through the workflow using the shared utility
      for (const request of createdRequests || []) {
        await initializeWorkflow({
          nominatorId: user?.id || '',
          employeeId: request.requester_id,
          requestId: request.id,
          courseName: selectedCourseData?.name_en || 'training',
          isExtendedWorkflow,
        });
      }
    ...
    onSuccess: (data) => {
      ...
      toast({
        ...
        description: isExtendedWorkflow 
          ? `${data?.length || 0} nomination(s) submitted. Routing to next approver in the chain.`
            ...
            {requestsLoading || teamLoading ? (
              ...
              <div className="space-y-3">
                {teamRequests.map((request) => {
                  ...
                  return (
                    <div key={request.id} className="border rounded-lg p-3 space-y-2 bg-card">
                      <div className="flex items-start justify-between gap-2">
                        <div className="min-w-0 flex-1">
                          <p className="font-medium text-sm truncate">{request.course?.name_en}</p>
                      ...
                      <div className="flex items-center justify-between text-xs text-muted-foreground">
                        <div className="flex items-center gap-1">
                          ...
                          <span>{member?.first_name_en} {member?.last_name_en}</span>
                        </div>
                        <Badge variant={request.request_type === 'nomination' ? 'secondary' : 'outline'} className="text-xs">
                          {request.request_type === 'nomination' ? 'Nomination' : 'Self'}
                        </Badge>
                      </div>
                    </div>
                  );
