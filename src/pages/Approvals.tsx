...
export default function Approvals() {
        ...
        .select(`
          *,
          request:training_requests(
            *,
            course:courses(name_en, name_ar, delivery_mode, duration_days, training_location)
          )
        `)
  ...
  const processMutation = useMutation({
    mutationFn: async ({
      ...
      // Update request status if rejected
      if (status === 'rejected' && selectedApproval?.request?.id) {
        await supabase
          .from('training_requests')
          .update({ status: 'rejected' })
          .eq('id', selectedApproval.request.id);
      }
    },
    onSuccess: () => {
      toast({
